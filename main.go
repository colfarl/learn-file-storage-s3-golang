...
func main() {
	...
	configCli, err := config.LoadDefaultConfig(context.Background(), config.WithRegion(s3Region))
	if err != nil {
		log.Fatal("S3_REGION environment variable not set")
	}
	...
	cfg := apiConfig{
		...
		port:             port,
		client:			  s3.NewFromConfig(configCli),
	}
